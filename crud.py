...
async def get_posts(
    ...
    # Calculate likes_count for each post
    for post in posts:
...
async def create_post(db: AsyncSession, post: schemas.PostCreate, author_id: int):
    ...
    if VECTOR_SEARCH_AVAILABLE and vector_search_service:
        try:
            embedding = vector_search_service.generate_post_embedding(db_post)
            db_post.embedding = embedding
        except Exception as e:
            print(f"Warning: Could not generate embedding for post: {e}")
            db_post.embedding = None
    else:
        db_post.embedding = None
...
async def delete_post(db: AsyncSession, post_id: int):
    ...
    if db_post:
        ...
        likes_result = await db.execute(select(models.Like).filter(models.Like.post_id == post_id))
        likes = likes_result.scalars().all()
        for like in likes:
            await db.delete(like)
        
        # Delete associated comments
        comments_result = await db.execute(select(models.Comment).filter(models.Comment.post_id == post_id))
...
async def update_comment(db: AsyncSession, comment_id: int, comment_data: schemas.CommentUpdate):
...
async def add_post_to_album(db: AsyncSession, album_id: int, post_id: int):
    """Add a post to an album by updating the post's album_id"""
    db_post = await get_post(db, post_id)
    if db_post:
        db_post.album_id = album_id
        await db.commit()
        await db.refresh(db_post)
    return db_post

async def remove_post_from_album(db: AsyncSession, album_id: int, post_id: int):
    """Remove a post from an album by setting the post's album_id to None"""
    db_post = await get_post(db, post_id)
    if db_post and db_post.album_id == album_id:
        db_post.album_id = None
        await db.commit()
        await db.refresh(db_post)
    return db_post